...
\end{equation}
The strength is given by the maximum of the $\psi_\mathrm{max}$ and the extent by the latitude at which $\psi_\mathrm{max}$ becomes zero.
...
Considering the angular momentum budget of the Earth we are trying to answer the question why there is a meridional circulation in the atmosphere in the first place. An answer is provided by the axisymmetric theory of \textcite{heldNonlinearAxiallySymmetric1980}. It also provides a scaling for the HC strength and extent linking it directly to thermodynamical parameters. Since this theory ignores eddies, we are further investigating their effect on the HC in the next subsection. Since there is no unifying theory incorporating angular momentum conservation and angular momentum transport by baroclinic eddies, we introduce the energetic budget framework of the HC to explain why it changes. This is a more diagnostic theory which does not explain the existence of the HC from first-hand principles but rather takes the necessity transport energy poleward as granted. It can be inferred from observational data of the TOA radiative imbalance showing an excess of incoming radiative fluxes relative to the outgoing radiation at the equator and vice versa at higher latitudes. This transport by the the atmosphere can be split up into a mean component, i.e., the HC, and and eddy component.
...
\end{equation}
poses another condition for a HC solution with the zonal wind $u$, $a$ being the radius and $\Omega$ the rotational frequency of the Earth. The angular momentum conserving wind can be derived to
\begin{equation}
    u_M=\Omega a\frac{\sin^2\varphi}{\cos\varphi}.
\end{equation}
This allows us to infer, via the thermal wind relationship, a temperature profile $\theta_M$ (see Figure \ref{fig:held_hou_T}), which is now slaved to angular momentum conservation. Assuming that the energy is conserved in the upper branch and that the solution is continuous at the edge of the HC, one obtains a finite width of the HC
\begin{align}
...
\end{align}
with $g$ being the gravitational acceleration of Earth and $\tau$ the time scale at which $\theta$ relaxes to $\theta_E$.
\begin{figure}[H]
     \centering
     \begin{subfigure}[b]{0.45\textwidth}
         \centering
   \includegraphics[width=1.0\textwidth,keepaspectratio,trim=0 0 0 0, clip]{literature/temperature_vallis.png}
    \caption{ The temperature profiles}
...
%     \label{fig:held_hou_T}
% \end{figure}
The contribution of this theory is to explain why there is a HC with finite width in the first place. Subsequent studies have advanced this theory also to temperature profiles which have their maximum off the equator to account for seasonal variations \parencite{lindzenHadleyCirculationsZonally1988}. HC weaker in equinox conditions than observed. Suggestion that the HC in the annual mean is not close to equinox conditions but rather that the HC flips between solstitial states. More recent studies have explained the discrepancy using eddy permitting models \parencite{walkerResponseIdealizedHadley2005}. The strength of an annually averaged Hadley circulation is comparable to the Hadley circulation driven by an annually averaged heating. This highlights the importance of eddies to drive the HC strength, which will be covered in the next section.  
...
equinox+summer HC in eddy driven regime
winter HC in angular momentum conserving regime

Applying a Reynolds decomposition on the zonal and meridional wind of the zonal momentum equation and ignoring viscous forces as well as vertical advection we obtain 
\begin{equation}
...
This equation has two limit: For $Ro\to 1$, the eddy term on the RHS becomes negligible and our Hadley circulation is close to angular momentum conservation
\begin{align}
    \frac{\partial \bar{M}}{\partial \varphi} = 0
\end{align}
which is described by the axisymmetric solution in Section \ref{sec:axisymmetric}. For $Ro\to 0$, the meridional wind is balanced by the eddy momentum flux convergence. This regime can be understood as eddy-driven. During the seasonal cycle the Rossby number varies, which determines how close to the angular momentum conserving limit the HC is. During the equinoxes the Rossby numbers are small which indicates a strong eddy influence. During the solstices the Rossby number is closer to $1$ in the winter HC but still small in the summer HC.
Assuming that the HC is entirely eddy driven ($Ro=0$), the HC strength can be predicted from the eddy momentum flux by inserting Equation \eqref{eq:eddy_divergence} into the Equation \eqref{eq:msf}. Similar to \textcite{walkerEddyInfluencesHadley2006}, this gives  
\begin{equation}
...
\end{equation}
with $p_\mathrm{max}$ being the pressure level at which the MSF becomes maximum and the Coriolis parameter $f$.
